...
aws ec2 create-key-pair --key-name keypair3 --query 'KeyMaterial' --output text > keypair3.pem
...
OPENSWAN_ID=$(aws ec2 run-instances --image-id ami-0efbfb724e7dd9f77 --instance-type t2.micro --key-name keypair3 --security-group-ids $VPN_SG --subnet-id $PUBLIC_SUBNET_ID --query 'Instances[*].InstanceId' --output text)
